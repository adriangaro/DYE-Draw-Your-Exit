...
public class Draw : MonoBehaviour {
	...
	void DrawObject () {
		...
		if (isMousePressed) {
			if(first){
				...
				auxiliarTexture = new Texture2D((int)((maxX - minX  + radius * 0.02f) / 0.01f), (int)((maxY - minY + radius * 0.02f) / 0.01f), TextureFormat.ARGB32, true);
	...
	void SetPixelsFromTexture(int radius, int x, int y){
		...
		Texture2D utilityTexture = new Texture2D(texture.width * 3, texture.height * 3);
